...
def parse_firmware_file(file: UploadFile) -> Dict:
    if not file.filename.endswith(".json"):
        raise HTTPException(status_code=400, detail="Invalid file type. Only JSON firmware files are supported.")

    content = file.file.read().decode("utf-8")
    try:
        return json.loads(content)
    except json.JSONDecodeError:
        raise HTTPException(status_code=400, detail="Firmware file is not valid JSON.")
...
async def upload_firmware(
    ...
    firmware_data = parse_firmware_file(file)
    checksum = firmware_data.get("checksum")
    if not checksum or checksum != "abcd1234efgh5678":
        log_entry["status"] = "Rejected - Checksum Mismatch"
        firmware_audit_log.append(log_entry)
        return JSONResponse(
            status_code=400,
            content={
                "status": "Rejected",
                "message": "🔴 Firmware checksum invalid or missing.",
                "blocked": True
            }
        )
    ...
    if firmwareVersion < entry["current"] and entry["rollback_protection"] and not attemptDowngrade:
        log_entry["status"] = "Blocked - Downgrade Not Allowed"
        firmware_audit_log.append(log_entry)
        return JSONResponse(
            status_code=400,
